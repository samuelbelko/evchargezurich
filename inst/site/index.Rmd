...
Die Daten liegen in CSV-Format vor. Die Datei enthält `r nrow(df)` Zeilen und die folgenden Spalten:

```{r, echo=FALSE}
names(df)
```
Hier sind die ersten 10 Zeilen der Daten:
```{r, echo=FALSE}
head(df,10)  %>%
  kbl(booktabs = TRUE) %>%
